...
static void
SNVTable_dealloc(SNVTableObject* const restrict self)
{
    ...
    if (NULL != list)
    {
        ...
        for (size_t i = 0; i < n; ++i)
        {
            ...
            if (NULL == vrd_avl_insert(subset, sample_id))
            {
                vrd_avl_destroy(&subset);
                vrd_pool_destroy(&pool);
                PyErr_SetString(PyExc_RuntimeError, "SNVTable.query(): vrd_avl_insert() failed");
                return NULL;
            } // if
        } // for
    } // if
